...
public class TokenAuthenticationMiddleware
{
    ...
    public async Task Invoke(HttpContext context, ITokenRepository tokenRepository)
    {
        ...
        {
            var token = await tokenRepository.FindOneAsync(new TokenFilter { Value = accessToken, AsNoTracking = true },
                x => new UserTokenModel { UserId = x.UserId, UserRole = x.User.Role, ExpireAt = x.ExpireAt });
