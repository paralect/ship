...
With auto-discovery routing, `auth` is applied automatically to all endpoints by default. To make an endpoint public (skip auth), export the `isPublic` sentinel:
