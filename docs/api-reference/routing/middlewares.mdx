...
With auto-discovery routing, `auth` is applied automatically to all endpoints by default. To make an endpoint public (skip auth), include the `isPublic` middleware in the `middlewares` array:
...
<Note>
`isPublic` is a no-op middleware used as a sentinel — the route registration checks for it by reference identity and skips `auth` when present.
</Note>
...
Use `isAdmin` middleware in your endpoint:
