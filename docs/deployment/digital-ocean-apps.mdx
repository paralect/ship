...
P.S. - Please note that the generated password starts with a letter, if not, regenerate the password. (otherwise, DigitalOcean can incorrectly parse mongoURI)
...
### Initial step
1. Select GitHub as a service provider. You might need to grant Digital Ocean access to your GitHub account/organization.
...
Click **Generate new token** and choose name and expiration period also choose read and write options for scopes.
