...
<Steps>
  <Step title="Download cluster's kubeconfig">
    ...
    <Warning>
      Remember to replace initial **Read only** token with new **Full access** token from [Personal access token](#personal-access-token) section.
    </Warning>
    ...
    - name: do-nyc3-my-ship-app-admin
      user:
        # replace initial read only token with new full access token from Personal Access Token section
        token: dop_v1_...
    ```
  </Step>
...
`KUBE_CONFIG_PRODUCTION` (`KUBE_CONFIG_STAGING` for staging environment)
