...
<Steps>
  ...
  <Step title="Integrate with created Container Registry">
    Go to the Container Registry's settings and find `DigitalOcean Kubernetes integration` section.

    You need to select your created `my-ship-app` cluster.
...
[Configure](https://docs.github.com/en/actions/security-for-github-actions/security-guides/using-secrets-in-github-actions#creating-secrets-for-a-repository) the following secrets (and variables if needed) in your GitHub repository:

| Name                    | Type     | Description                                                                                                                 |
|-------------------------|----------|-----------------------------------------------------------------------------------------------------------------------------|
| DIGITAL_OCEAN_TOKEN     | secret   | The secret access user created for CI/CD. This allows GitHub Actions to authenticate with DO services                       |
| KUBE_CONFIG_STAGING     | secret   | This kubeconfig file contains the credentials to access the staging cluster. Copy of `my-ship-app-kubeconfig.yaml` from [here](#accessing-cluster-from-a-local-machine)         |
| KUBE_CONFIG_PRODUCTION  | secret   | This kubeconfig file contains the credentials to access the production cluster. Copy of `my-ship-app-kubeconfig.yaml` from [here](#accessing-cluster-from-a-local-machine)       |
...
<Note>
  We set up **DIGITAL_OCEAN_TOKEN** to be universal for both production and staging environments with **Full access** scope.
</Note>
