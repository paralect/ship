...
Now we need to give EKS permissions to our user. Use the following command to attach the user to kubernetes mains group:

```shell
eksctl create iamidentitymapping --cluster my-app --arn arn:aws:iam::402167441269:user/cicd --group system:mains --username cicd
```
