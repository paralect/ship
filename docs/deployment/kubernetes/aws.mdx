...
```json api/src/config/environment/production.json
{
...
```json web/src/config/environment/production.json
{
  "apiUrl": "https://my-app-api.paralect.com",
  "webUrl": "https://my-app.paralect.com",
}
...
To begin, create a namespace according to your deployment environment:

```shell
kubectl create namespace production
```

Then, add secrets using the following command. Remember to customize my-secrets with your preferred secret name:

```shell
kubectl create -n production secret generic ship-secrets --from-literal=MONGO_URI='mongo_uri' --from-literal=MONGO_DB_NAME=mongo_db_name --from-literal=API_URL=api_url --from-literal=WEB_URL=web_url
```

Replace variables with your values and **ship-secrets** with your preferred secret name.

After that, navigate to deployment.yaml, job.yaml files in api folder and deployment.yaml file in scheduler folder and update the secret names in the env section:

```yaml deploy/app/api/template/deployment.yaml
...
```yaml deploy/app/api/template/job.yaml
        imagePullPolicy: Always
          env:
            - name: MONGO_URI
              valueFrom:
...
```yaml deploy/app/scheduler/template/deployment.yaml
        imagePullPolicy: Always
        env:
          - name: MONGO_URI
            valueFrom:
...
Make sure the names of the secrets are the same everywhere in your setup to keep everything consistent.
