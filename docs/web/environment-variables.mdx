...
This setup is found in the `src/config/index.tsx` file.
...
<Steps>
    ...
    <Step title="Modify the Schema in `src/config/index.tsx`">
        Update the schema in the `config/index.tsx` file to include the new variable.
        This step is crucial for schema validation.

        ```typescript config/index.tsx
    ...
    <Step title="Update the Configuration Object">
        Add the new variable to the configuration object within the same file (`config/index.tsx`), ensuring it matches the schema.
