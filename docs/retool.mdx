...
2. Import prebuilt applications from retool [folder](https://github.com/paralect/ship/tree/main/services/api-koa/src/config/retool) in Ship. Open `Apps` > ` Create New` > `From JSON` and choose needed.
...
4. Add `x-admin-key` header. The value for `x-admin-key` is stored [here](https://github.com/paralect/ship/blob/main/services/api-koa/src/config/environment/staging.json) as `adminKey`.
...
4. Write `Connection String` of your database.  The value for `Connection String` is stored [here](https://github.com/paralect/ship/blob/main/services/api-koa/src/config/environment/staging.json) as `mongo.connection`.
