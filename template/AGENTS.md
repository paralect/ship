...
| `agent_docs/api_resource_and_endpoint_workflow.md` | Adding/modifying API resources or procedures |
...
- Forget to update barrels after adding a new procedure file — add the export to `procedures/index.ts`. For new resources, also add to `src/router.ts`.

---

## Type Flow: API → Web

1. API procedures define `.input(zodSchema).output(zodSchema).handler(...)`.
...
- [ ] If API procedures/schemas changed → `pnpm --filter api build:types` ran
