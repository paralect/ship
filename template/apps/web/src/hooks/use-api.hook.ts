...
export function useApiQuery<T extends AnyClient>(
  ...
  if (rest.length === 2) {
    ...
    if (
      arg &&
      typeof arg === 'object' &&
      ('enabled' in (arg as Record<string, unknown>) ||
        'staleTime' in (arg as Record<string, unknown>) ||
        'refetchOnWindowFocus' in (arg as Record<string, unknown>))
    ) {
...
type UseApiMutationOptions<TResponse, TParams> = Omit<UseMutationOptions<TResponse, ORPCError, TParams>, 'mutationFn'>;
